# Call #36 — Claim Paid to the Provider, Member Already Paid Too

- **Agent:** Michael
- **Caller:** MEMBER
- **Tier:** AVERAGE
- **Score:** 73/100
- **Sentiment Arc:** CONFUSED → SATISFIED
- **Resolution:** RESOLVED
- **Date:** 2026-09-09
- **Start:** 11:55:30
- **End:** 12:03:07
- **AHT:** 7m 37s
- **Duration:** ~8 min
- **Queue:** Claims & EOB
- **Repeat Contact:** follows an unresolved call on Tue 08 Sep 2026: Coverage Is Expiring — Member Does Not Know Why
- **Topics:** duplicate payment · refund · assignment of benefits · provider

**Member context:** Terrence Boyd, 47 · Member ID CB-5519074 · MetLife dental · double payment

## Transcript

Agent Michael: Choice Administrators, Michael speaking. This call may be recorded for quality and training. Can I take your name and member ID?

Caller: Terrence Boyd, member ID CB-5519074.

Agent Michael: Thank you. I have the record open — go ahead.

Caller: I called on the 8th about termination and nobody came back to me, so you should know I'm already on my second attempt. I paid my dentist $260 at the appointment, and now my EOB says you also paid them $208. Have they been paid twice?

Agent Michael: Let me look. The claim processed with the allowed amount at $260. Plan paid $208, member responsibility $52.

Caller: So I should have paid $52, not $260.

Agent Michael: Correct. The practice collected the full amount upfront rather than waiting for the claim, which some do. They now owe you the difference — $208.

Caller: Will they just send it?

Agent Michael: Usually, but not always automatically. Call their billing office, reference the claim date, and tell them the EOB shows member responsibility of $52 against $260 collected.

Caller: And if they don't?

Agent Michael: Call us back. Network providers agree not to collect more than the member responsibility, so persistent overcollection is a contract issue we can raise with them.

Caller: Good to know there's a backstop.

Agent Michael: There is. Reference OP-2026-00441 if you need to come back.

Caller: Thank you.

## AI Insights Panel — NanoVox 5-Layer Output

L1 — Transcription & understanding: Call type: Provider overcollection. Caller: MEMBER (Terrence Boyd, CB-5519074). Tone:
confused → satisfied. Duration: 8 min. Repeat contact — prior call in period closed unresolved.
L2 — Call insights: Provider collected $260 upfront against a $52 member responsibility. Agent explained the overcollection, gave the
member specific language for the refund request, and confirmed the network contract backstop. Resolution: RESOLVED.
L3 — Agent quality: Agent Score: 73/100. POSITIVE: identified the overcollection clearly (+), gave specific language for the provider
conversation (+), explained the network contract protection as a backstop (+), reference provided (+). NEGATIVE: did not offer to
contact the provider directly (-), did not log the practice for a collection-practice pattern check (-), no written EOB summary offered (-).
L4 — Operational BI: PROVIDER PERFORMANCE · Owner: Provider Relations — upfront full-fee collection ahead of adjudication is a
contract compliance issue and shifts the refund burden onto members. Recommend tracking practices by overcollection reports and
addressing repeat offenders. REPEAT CONTACT: same member (CB-5519074) called on Tue 08 Sep 2026 and that call closed
unresolved. Repeat contact within the period is a retention signal in its own right and should be counted against the earlier call, not this
one.
L5 — Real-time assist: Fired at 2:00 — member payment versus member responsibility mismatch surfaced the overcollection. Provider
pattern logging action not surfaced; recommend adding.
