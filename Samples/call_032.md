# Call #32 — Member Asks Whether Coverage Continues After Leaving — Told to Call Back
...
Agent Ryan: Choice Administrators, Ryan.
 Caller: My last day is next Friday. What happens to my dental and vision? Member ID CB-8819004.
...
Duration: 5 min. Member mid-orthodontic treatment.
...
UNRESOLVED.
L3 — Agent quality: Agent Score: 33/100. NEGATIVE: could not state end-of-month coverage rule with confidence (-), did not know the
COBRA election period (-), did not recognise that mid-treatment orthodontia is a material continuation consideration (-), gave no dates on a
time-sensitive question (-), no case opened (-), no callback (-). POSITIVE: none identified.
...
benefit termination on coverage loss — this should trigger a specific continuation conversation, not a referral.
L5 — Real-time assist: SHOULD HAVE FIRED at 1:00 — pending termination plus active orthodontia instalment schedule is a continuation
trigger. Should have surfaced end date, COBRA election window, and the orthodontia payment implication. No rule configured.
