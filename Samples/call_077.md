# Call #77 — Adding a Newly Enrolled Dependent to the Portal View
...
Agent Nicole: Yes. Coverage is active from the 1st regardless of what the portal displays. If she needs to be seen before the refresh, give the provider her name, date of birth, and your subscriber ID and they can verify directly with Delta.
...
Root cause: weekly portal refresh cycle. Repeat contact — prior call in period closed unresolved.
...
concern (+), gave a workaround for immediate provider verification (+), sent a card unprompted (+). NEGATIVE: did not flag the refresh
